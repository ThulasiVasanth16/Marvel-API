...
const imageArray = ["img2.jpg", "img3.jpg", "img1.jpg", "img0.jpg", "img4.jpg"];
...
requestCharacter = () => {
    ...
    const url = `http://gateway.marvel.com/v1/public/characters?nameStartsWith=${characterInput.value}&limit=10&ts=1&apikey=${apikey}&hash=${hash}`;
...
loadCharacter = (data) => {
    characters.style.backgroundImage = 'linear-gradient(rgba(0,0,0,0.6),black), url(comics.jpg)';
    for (let i = 0; i < data.data.results.length; i++) {
        if (!data.data.results[i].thumbnail.path.includes("image_not_available")) {
            ...
            Character.appendChild(dicCharacter);
...
loadComics = (data) => {
    ...
    for (let i = 0; i < data.data.results.length; i++) {
        if (!data.data.results[i].thumbnail.path.includes("images_not_available")) {
            var image_path = data.data.results[i].thumbnail.path + '/portrait_uncanny.jpg';
